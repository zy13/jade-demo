/**
 * @Author: Jet.Chan
 * @Date:   2016-12-22T17:51:54+08:00
 * @Email:  guanjie.chen@talebase.com
* @Last modified by:   Jet.Chan
* @Last modified time: 2017-01-11T21:56:43+08:00
 */

import cui from 'c-ui';
import './commonStyle/style.less'
import './objective_fill.less'
import './objective_fill_edit.less'
...
//客观填空题
import tpl_objective_fill from './tpl/objective_fill.html';
const juicerTpl = juicer(tpl_objective_fill);
import tplEdit_objective_fill_edit from './tpl/objective_fill_edit.html';
const juicerTplEdit = juicer(tplEdit_objective_fill_edit);
...
class objective_fill extends baseClass {
    ...
    showPreview(preViewDataConf, $editTpl) {
        ...
        preViewDataConf.setHeight = parseInt(height)
    ...
    validate($tpl){
        let validate = true,
            $height = $('.ques-setting .layout .height input[type=txt]',$tpl),
            $width = $('.ques-setting .layout .width input[type=txt]',$tpl),
            $limit = $('.ques-setting .layout .limit input[type=txt]',$tpl),
    ...
    showEdit(displaceTpl, dataConf) {
        ...
        $editTpl.find('.cui-textBoxContainer').each(function () {
            new cui.TextBox($(this))
        })
        ...
        $editTpl.on('click', '.cpf-icon-thin-close.del-option', (e) => {
            ...
            $optItem.nextAll('.options-item').find('.option-index').each((i,v)=>{
        ...
        $editTpl.on('focus', '.ques-setting .layout input[type=txt]', (e) => {
            ...
            .on('blur', '.ques-setting .layout .height input[type=txt]', (e) => {
                let height = $(e.target).val() //最小高度为40
                if (height != '') {
                    height = parseInt(height)
                    if (height < 40) { //判断大小
                        $(e.target).val('40')
                        new WarnTip($(e.target).parent(), '最小高度为40', {
                            width: '80px'
                        })
                    } else if(height>1000){
                        $(e.target).val('1000')
                        new WarnTip($(e.target).parent(), '最大高度为1000', {
                            width: '90px'
                        })
                    }else{
                        $(e.target).val(height)
                    ...
                    // $(e.target).val('40')
                    new WarnTip($(e.target).parent(), '不能为空', {
                        width: '50px'
                    })
                }
            })
            .on('blur', '.ques-setting .layout .width input[type=txt]', (e) => {
                let width = $(e.target).val() //最小高度为150，最大宽度为820
                if (width != '') {
                    width = parseInt(width)
                    if (width < 150) {
                        $(e.target).val('150')
                        new WarnTip($(e.target).parent(), '最小宽度为150', {
                            width: '85px'
                        })
                    } else if (width > 820) {
                        $(e.target).val('820')
                        new WarnTip($(e.target).parent(), '最大宽度为820', {
                            width: '55px'
                        })
                    } else {
                        $(e.target).val(width)
            ...
            .on('blur keyup input', '.ques-setting .layout .limit input[type=txt]', (e) => {
                ...
                if (limit != '') {
                    ...
                    if (limit == 0) {
                        $(e.target).val('')
                        new WarnTip($(e.target).parent(), '字数上限不能为0', {
                            width: '80px'
                        })
                    }else if (limit > 5000) {
                        $(e.target).val('')
                        new WarnTip($(e.target).parent(), '上限不能超过5000', {
                            width: '110px'
                        })
                    } else {
                        $(e.target).val(limit)
